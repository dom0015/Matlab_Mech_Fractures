  ...
  SUBROUTINE zagmg_FlexCG(N,f,X,ITER,RESID,a,ja,ia,ijb)
    ...
    DO WHILE ( ITER < MAXIT .AND. RESID > TOL )
       ...
       IF ( ITER > 1 ) THEN
          ...
          flop=flop+dble(4*N)
          IF (intmv) THEN
              CALL ZAXPY(NBLAP, BET1, SD, IONE, S(1+4*N), IONE)
              flop=flop+dble(2*N)
          END IF
       ENDIF
       CALL ZCOPY(NBLAP, S(1+3*N), IONE, S(1+2*N), IONE)
       IF (intmv)  THEN
           CALL ZCOPY(NBLAP, S(1+4*N), IONE, SD, IONE)
       ELSE
...
310 CONTINUE
   ...
   !
    ELSE IF (nrst > 1) THEN
   !
       CALL zagmg_GCR(n,f,x,iter,resid,a,ja,ia,ijb)
   !
   !
    ELSE
   !
       CALL zagmg_FlexCG(n,f,x,iter,resid,a,ja,ia,ijb)
   !
   !
    END IF
!
