...
! Release 3.2.4-aca built on "Oct 12 2016" by Yvan Notay
!
...
!
    ...
    REAL(kind(0.0d0)) :: memi=0.0,memax=0.0,memr=0.0,mritr,rlenilen,smoothtp,omeg=1.0
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmg_FlexCG(N,f,X,ITER,RESID,a,ja,ia,ijb)
    ...
    kstat=0
    init=.FALSE.
    IF (ijb .GE. 10) init=.TRUE.
    intmv=.FALSE.
    IF (mod(ijb,10).EQ.0 .AND. nlev.GT.1) intmv=.TRUE.
    IF (wfo) THEN
    ...
    DO WHILE ( ITER < MAXIT .AND. RESID > TOL )
       ...
       IF ( ITER > 1 ) THEN
          ...
          flop=flop+dble(4*N)
          IF (intmv) THEN
              CALL DAXPY(NBLAP, BET1, SD, IONE, S(1+4*N), IONE)
              flop=flop+dble(2*N)
          END IF
       ENDIF
       CALL DCOPY(NBLAP, S(1+3*N), IONE, S(1+2*N), IONE)
       IF (intmv)  THEN
           CALL DCOPY(NBLAP, S(1+4*N), IONE, SD, IONE)
       ELSE
...
310 CONTINUE
   ...
   !
    ELSE IF (nrst > 1) THEN
   !
       CALL dagmg_GCR(n,f,x,iter,resid,a,ja,ia,ijb)
   !
   !
    ELSE
   !
       CALL dagmg_FlexCG(n,f,x,iter,resid,a,ja,ia,ijb)
   !
   !
    END IF
!
