...
PRESSURE{1}'
%% VISUALIZATION ETC.
Q = extract_flow( A, u0(1:length(A)), POINTS, ELEMENTS, Nxy-1, Dirichlet_windows, downEdge, rightEdge, upEdge, leftEdge );
startx=10*ones(10,1);
starty=linspace(5,10,10);
[ tmp1,tmp2,xx,yy,ftmp1,ftmp2,fxx,fyy,node_fluxx,node_fluxy] = streamlines_calc( u0(1:length(A)),POINTS,double(ELEMENTS),linspace(0.1,9.9,23),linspace(0.1,9.9,23),...
...
figure;
N=length(A);
h = trisurf(ELEMENTS,POINTS(:,1),POINTS(:,2),u0(1:N));
alpha 0.8
h.EdgeColor = 'none';
colormap jet(1000)
colorbar
