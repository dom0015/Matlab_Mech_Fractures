function [ node,elem,bdflag,fractures,fracture_matrice,Db] = multi_fracture_tear_FETI( node,elem,fractures,bdflag,fracture_matrice,Db)
...
for fracture_id=1:m
    ...
    for i=2:n-1
        ...
        for nd=1:nn
            ...
            [ under_fracture ] = mesh.my_elem_classification( node(fracture{i-1}(1),:),node(node_id,:),node(fracture{i+1}(1),:),elem_t(cor_elems,:));
            if (sum(under_fracture)==0)||(sum(under_fracture)==length(under_fracture))
                
            else
            [ fractures ] = mesh.duplicate_node_in_other_fractures( fractures,fracture_id,node_id,nodes_count+1 );
            ...
            fracture_matrice = mesh.update_fracture_matrice(fracture_matrice,node,fracture_id,node_id,nodes_count+1,node(fracture{i-1}(1),:),node(fracture{i+1}(1),:));
    ...
    [ fractures ] = mesh.remap_fracture_nodeid( fractures,nodes_map );
