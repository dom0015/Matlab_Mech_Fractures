function [ B,G,Au ] = matrices_modif2( fracture_matrice,node )
...
for i=1:length(fracture_matrice)
    ...
    N_f_elem = N_f_elem + (length(fracture_matrice{i}.under_material)+1)*2;
end
...
for fracture_id=1:n
    for side=1:2
        ...
        idx_f_elem=[((counter_f_elem+1):(counter_f_elem+len))' ((counter_f_elem+2):(counter_f_elem+len+1))'];
        counter_f_elem = counter_f_elem+len+1;
        ...
        for i=1:2
            for j=1:2
                ii = idx_f_elem(:,i);
                jj = idx_d_node(:,j);
                B = B + sparse(ii,jj,M_local(i,j)*LENGTHS,N_f_elem,N_d_node);
    ...
    %             G = G + sparse(ii,jj,M_local(i)*LENGTHS.*ALPHA,N_f_node, N_f_elem);
        ...
        for i=1:2
            for j=1:2
                ii = idx_f_elem(:,i);
                jj = idx_f_elem(:,j);
                Au = Au + sparse(ii,jj,M_local(i,j)*LENGTHS./ALPHA,N_f_elem,N_f_elem);
            end
        end
    end
end
